...
var PUDIM = window.PUDIM = (function () {
    ...
    function objectToRows(obj) {
        ...
        Object.keys(obj)
            .filter(function (key) {
                return key[0] !== '_' && obj.hasOwnProperty(key);
            })
            .map(function (key) {
                return {
                    key: key,
                    friendly: metadata[key] && metadata[key].name || key,
                    value: obj[key]
                };
            })
            .sort(function (a, b) {
                var matchA = rx.exec(a.friendly), matchB = rx.exec(b.friendly);
                if (matchA && matchB && matchA[1] === matchB[1]) {
                    a = {friendly: Number(matchA[2])};
                    b = {friendly: Number(matchB[2])};
                }
                return a.friendly < b.friendly ? -1 : a.friendly === b.friendly ? 0 : 1;
            })
            .forEach(function (data) {
                var key = data.key, value = data.value, friendly = data.friendly;
                html.push('<td class="key" title="' + key + '">' + friendly + '</td><td class="value" title="' + value + '">' + decode(value) + '</td>');
            });
